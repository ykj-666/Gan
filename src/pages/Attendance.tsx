...
export default function AttendancePage() {
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-2xl border border-gray-200 bg-white p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-semibold text-gray-900">请假与出差考勤</h1>
            <p className="mt-1 text-sm text-gray-500">
              请假管理与出差考勤统计统一收敛到同一页面，但保留各自模块能力。
            </p>
          </div>
        </div>
      </div>
      ...
      >
        <TabsList className="grid h-auto w-full max-w-[420px] grid-cols-2 rounded-xl bg-white p-1 shadow-sm">
          <TabsTrigger value="leave" className="gap-2 py-2.5">
            <CalendarCheck className="h-4 w-4" />
            请假管理
          </TabsTrigger>
          <TabsTrigger value="trip" className="gap-2 py-2.5">
        ...
        <TabsContent value="trip" className="mt-5">
          <BusinessTripSection />
        </TabsContent>
      </Tabs>
    </div>
